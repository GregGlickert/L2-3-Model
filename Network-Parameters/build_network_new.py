...
num_PN_sum = np.cumsum(num_PN)
assy_ids = np.empty(num_PN_sum[-1], dtype=int)
for i, idx in enumerate(assign_assembly(num_PN_sum[-1], n_assemblies, rng=rng)):
    assy_ids[idx] = i
assy_ids = np.split(assy_ids, num_PN_sum[:-1])

if edge_effects:
    virt_num_PN_sum = np.cumsum(virt_num_PN)
    virt_assy_ids = np.empty(virt_num_PN_sum[-1], dtype=int)
    for i, idx in enumerate(assign_assembly(virt_num_PN_sum[-1], n_assemblies, rng=rng)):
        virt_assy_ids[idx] = i
    virt_assy_ids = np.split(virt_assy_ids, virt_num_PN_sum[:-1])
...
assy_pop_name = {'PN'}  # pop_name of populations that form assemblies
